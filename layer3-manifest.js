...
class LayerManifest {
    ...
    /**
     * Resolves the dependency execution order (Topological Sort)
     * @returns {string[]} Array of layer IDs in execution order
     */
    resolveExecutionOrder() {
        ...
        const visit = (layerId) => {
            if (tempVisited.has(layerId)) {
                console.warn(`[Layer 3] Circular dependency detected: ${layerId}`);
                return;
            }
            ...
            if (layer && layer.dependencies) {
                layer.dependencies.forEach(depId => {
                    if (this.layerMap.has(depId)) {
                        visit(depId);
                    } else {
                        console.warn(`[Layer 3] Missing dependency: ${depId} for ${layerId}`);
                    }
                });
            }
        ...
        for (const layerId of this.layerMap.keys()) {
            visit(layerId);
        }
